...
- **A button that cannot be built correctly is omitted, not approximated.** `alertKeyboard` returns `null` for a non-uuid id. Telegram answers a malformed button url with a 400, `sendTelegramMessage` treats 400 as non-retryable, and `notifyPendingAlerts` then releases the claim and re-sends the identical broken message forever — so a bad link costs the whole alert, not just the button. Same reason `InlineKeyboardMarkup` in `sendMessage.ts` is narrowed to url buttons: Telegram's real button type is a union where exactly one field may be set, and a wider type here would let a caller compile a 400.
...
- **"Today" is the API's UTC day, and the client must agree with it.** `todayUtcIso` in `src/lib/forecast.ts` mirrors `liveForecast.ts`; both Open-Meteo calls request `timezone=UTC`. When no row matches, the screen says so — it must never fall back to the first row, which relabels tomorrow's numbers as today's. The underlying wrongness for a user in the Americas is real and is tracked as `miniapp-design-v1.md` §10.5; fixing it is server-side, not a client workaround.
