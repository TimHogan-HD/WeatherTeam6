...
- **Only the ensemble run stores `raw`.** Every deterministic variable requested has a column on `weather_run_hours`, so its parsed rows are the whole payload, and one response covering six models would otherwise be stored six times to preserve nothing. Retention is 14 days parsed / 48h raw (`pruneRuns.ts`), and a trip four weeks out therefore has no trend history until it comes inside the window — an accepted cost, not a bug.
