...
4. Does this add a client surface? `apps/miniapp` is the only client, and its surface is four routes — `/login`, `/`, `/location/:id`, `/add`. Radar, walls, trips, and shade map are out of scope; `/add` is **in** scope and its API is already built.
