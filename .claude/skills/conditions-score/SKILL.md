...
- **Known issue #21, half fixed.** The temp component is capped at 12 of 100 points and saturates above 35 °C, so a location at 39.9 °C (unclimbable) still scores ~85. **The scoring half is still open.** The copy half was fixed on 2026-08-26: no surface maps a score to an opinion any more, and `summarizeConditions` in `packages/types/src/conditionsCopy.ts` suppresses the score as a summary whenever a component is 0 or a Severe+ alert is active. Use that function — do not write a new score-to-text mapping anywhere.
