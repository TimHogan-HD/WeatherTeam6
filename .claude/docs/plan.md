...
> `apps/api` is live on Vercel + Neon, `apps/mobile` is archived, and `apps/miniapp`
> exists as of 2026-08-25 — the Task 5 shell, not yet deployed or registered with
> @BotFather. See `.claude/docs/session-notes.md` for the running record.
...
| **Task 5 — Mini App shell** | 🟡 **Code complete — merged `b06ebed` (PR #38) 2026-08-25, not yet deployed.** `apps/miniapp` exists — Vite + React static build, `vercel.json` SPA rewrite, `telegram-web-app.js` loaded ahead of the app module, `ready()`/`expand()`, and chrome colours gated per-method (`setBackgroundColor` at Bot API 6.1, `setHeaderColor` at 6.9, no `bg_color` fallback). Ships the §0a token adapter (`src/theme/tokens.css.ts` + generated `:root` custom properties), all three §2 routes with Telegram's `BackButton` as the only back affordance, and the §5 React Query defaults. Screens are placeholders — Task 6. **Still outstanding, and both need the repo owner:** create the separate Vercel project (root `apps/miniapp`, "include files outside root dir" on, preset **Vite**, no `NODE_ENV`) and register the production URL with @BotFather. Origin lockdown means production domain only; preview URLs will not open. Until those two are done the acceptance criterion — the menu button opening the app inside Telegram — is unmet. Setup steps: `apps/miniapp/README.md`. |
