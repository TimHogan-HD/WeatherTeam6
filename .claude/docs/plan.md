...
| **Task 5a — Add-location API** | ✅ **Complete — 2026-08-25**, branch `claude/add-location-api`. Backend only, and a prerequisite for Task 6's add flow. Five changes, specified in `miniapp-design-v1.md` §12.3: `GET /geocode?q=` (Open-Meteo, keyless, proxied server-side), `GET /preview?lat=&lon=` (live forecast for an unsaved location — `computeLiveForecast` over a synthetic `LiveForecastLocation`, persists nothing), `POST /locations` accepting `is_climbing_location` + optional `rock_type` (it currently hardcodes `false` at `routes/locations.ts:174`), `DELETE /locations/:id` (**none exists today** — a save flow with no unsave is a trap), and **persisting `elevation_m` on save** (it never has — without it, preview and the saved location disagree on temperature by the full lapse-rate correction; §12.3 change 5). `CreateLocationInput` in `packages/types` changes with it. **Independent of the `initData` auth work**, so it does not wait on the constraint below. |
