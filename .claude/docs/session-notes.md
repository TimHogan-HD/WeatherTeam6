...
- **Re-run `npm run check:add-location` before trusting the add flow again.** The 16-check pass was against the scratchpad copy; the in-repo version was restructured afterwards (runtime imports deferred so a missing `DATABASE_URL` explains itself instead of throwing from `db/index.ts`). Same logic, but it has not been re-run against a database.
...
- **Re-run `npm run check:add-location` before trusting the add flow.** Still outstanding from Task 5a — the 16-check pass was against the scratchpad copy, and the in-repo version was restructured afterwards.
