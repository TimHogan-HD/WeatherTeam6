...
- The Task 5a acceptance script was written to the session scratchpad and **is gone once that is cleaned up**. If the save/delete path needs re-testing later — Task 6 will touch it — rewrite it or promote it to `apps/api/src/scripts/`, which is where `importCrags.ts` already lives.
