...
- **`GET /hourly/:locationId` also carries the v2 scoring model's readings** as of 2026-09-21 (`HourlySeries.readings`). Per hour: a rock reading (`wet`/`drying`/`dry`), a friction reading (`poor`→`great`), a 0-100 score, the rock surface temperature and the dew-point margin. Per local day: the best contiguous window and the day's best hour, both chosen server-side. **Two things a client must not get wrong.** `readings.model` is always `gfs_seamless` and may differ from `model` above — irradiance is never pooled (issue #155) — so do not attribute a reading to the model heading the weather columns. And the **0-1 factors behind the readings are deliberately absent**: words and ordering only, never a magnitude, because the friction reading rests on one unvalidated step (`.claude/rules/architecture.md`). A non-crag gets `unavailable_reason: 'not_a_climbing_location'` and no readings.
- **`GET /forecast/:id` carries per-day scores** — `score`, `confidence`, `unavailable_reason` and five `component_*` fields. **Nothing renders any of them as of 2026-09-21** (scoring v2 Phase 3b): the daily list reads per-day v2 scores off `HourlySeries.readings` instead, and `summarizeConditions` — which is what made the `component_*` fields load-bearing, for a "limited by X" qualifier — has been deleted. The fields are still sent and Phase 5 removes them with the scorer.
- **`GET /conditions/:locationId` carries today's v2 readings** as of 2026-09-21, as `ConditionsScore.readings` (`ConditionsReadings` in `packages/types/src/hourly.ts`): the model, the hour covering now, today's window and best hour, and the location's own `utc_offset_seconds`. It exists so the list card and the detail screen read one endpoint — the two models disagree by around thirty points on a hot day, and one crag must not carry two numbers on two screens. The field is **optional on the type**: an absent one means the API is older than the client, which is not the same as the model having nothing to say, and a client renders nothing rather than inventing a reason. A non-crag is not asked at all.
